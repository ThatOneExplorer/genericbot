...
module.exports ={
    ...
    async execute(messageCreate){
        ...
        if(!member){
            ...
            .setDescription(`Is the user in the guild? Does the user exist? I can't kick someone who isn't in the server :/`)
            .setColor("Red")
        ...
        if(!reason){
            ...
            .setDescription(`You need to provide a reason to kick them for :/`)
            .setColor("Red")
            return messageCreate.reply({embeds: [noreason]})
        }

        if(!member.kickable){
let notkickable = new Discord.EmbedBuilder()
.setTitle(`Can not kick this user`)
.setDescription(`I am either missing the Kick Members permission or this user has a role that is higher then mine!`)
.setFooter({text:`It could also be a possibility that this is an issue with the bot itself, in this case please report it to a server administrator / owner.`})
.setColor("Red")
 
 return messageCreate.reply({embeds: [notkickable]})
        }
        ...
        let kick = new Discord.EmbedBuilder()
        .setTitle(`Successfully issued **KICK** to ${member.user.username}#${member.user.discriminator}`)
        .addFields(
            { name: 'Moderator', value: `${messageCreate.author.tag}`, inline: true }, { name: 'Reason', value: `${reason}`, inline: true },
        )        
        .setColor("Green")
     
    messageCreate.reply({embeds: [kick]})
    let kickdm = new Discord.EmbedBuilder()
    .setTitle(`You have been **KICKED** from ${messageCreate.guild.name}!`)
    .addFields(
        { name: 'Moderator', value: `${messageCreate.author.tag}`, inline: true }, { name: 'Reason', value: `${reason}`, inline: true },
    )
     .setFooter({text: `Punishment ID: ${punishmentCode}`})
    await member.send({embeds: [kickdm]}).catch(e => {messageCreate.channel.send(`Error occured while trying to dm ${member.username}#${member.discriminator}: ${e}`)})
    member.kick();
} catch(e){
    console.log(e)
