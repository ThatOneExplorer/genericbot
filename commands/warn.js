...
if (!member){
    ...
    .setDescription(`Is the user in the guild? Does the user exist? I can't warn someone who isn't in the server :/`)
    .setColor("Red")
...
if(!reason){
    let noreason = new Discord.EmbedBuilder()
        ...
        .setDescription(`You need to provide a reason to warn them for :/`)
        .setColor("Red")
...
// Example Usage:
const punishmentCode = generatePunishmentCode(10); // Generates a 10-character long code
console.log(punishmentCode);
...
let warn = new Discord.EmbedBuilder()
.setTitle(`Successfully issued **WARNING** to ${member.user.username}`)
.addFields(
    { name: 'Moderator', value: `${messageCreate.author.tag}`, inline: true }, { name: 'Reason', value: `${reason}`, inline: true },
)

.setColor("Green")
messageCreate.reply({embeds: [warn]})

let warndm = new Discord.EmbedBuilder()
.setTitle(`You have been **WARNED** in ${messageCreate.guild.name}!`)
.addFields(
    { name: 'Moderator', value: `${messageCreate.author.tag}`, inline: true }, { name: 'Reason', value: `${reason}`, inline: true },
)
.setFooter({text: `Punishment ID: ${punishmentCode}`})

return member.send({embeds: [warndm]}).catch(e => {messageCreate.channel.send(`Error occured while trying to dm ${member.username}: ${e}`)})
} catch(e){
    console.log(e)
